...
"""
function feltype end

feltype(a) = feltype(typeof(a))  # similar to eltype
# overwrite this one:
feltype(a::Type) = eltype(a)  # defaulting to eltype seems save

"""
  change_feltype(functor_instance or FunctorType, NewElementType)

return new type with inner element changed, or new instance with instance of type widened,
  depending on whether the given functor is a Type or a value respectively
"""
function change_feltype end

# have a version for types and values separately, as in Julia it is relatively common that the default value constructor 
# picks a too concrete type which we need to lift afterwards
function change_feltype_value end
function change_feltype_type end

change_feltype(T::Type, E::Type) = change_feltype_type(T, E)
change_feltype(t::T, E::Type) where T = change_feltype_value(t, E)

# we use a simple default implementaion
...
change_feltype_type(T, E) = T
change_feltype_value(t, E) = t

"""
  feltype_unionall_implementationdetails(functor)
...
"""
feltype_unionall_implementationdetails(functor) = change_feltype(functor, unionall_implementationdetails(feltype(typeof(functor))))
