"""
We define traits wrapper around the plain functions
these provide extra type-level functionality, however need to be initialized with ``Traits.@traitsof_link``

For educational purposes we separated the Traits-based implementation completely.
One downside is that certain functions need to be duplicated.
"""
...
# We define a functioname_traits function for each orginal function to enable dispatch on traits.
# This approach is recommended, because you may want to find traits on custom preprocessings like ``eltype()``, which cannot be abstracted generically.
# In addition, the intended use is clear for everyone. As you see the traits-based dispatch is just a special case of normal julia dispatch.

fforeach(traitsof::Traitsof, f::F, a::A) where {F, A} = fforeach_traits(traitsof, f, a, traitsof(F), traitsof(A))
# most importantly, default to the original function:
fforeach_traits(::Traitsof, args...; kwargs...) = fforeach(args...; kwargs...)

# orthogonal to being able to use traitsof in the function definition, we also want to make the function available as a trait
...
fmap_traits(::Traitsof, args...; kwargs...) = fmap(args...; kwargs...)
...
feltype_traits(::Traitsof, args...; kwargs...) = feltype(args...; kwargs...)
